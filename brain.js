// --- CONFIGURACIÓN DE FIREBASE ---
        ...
        const staticData = [
            ...
            { id: 17, nombre: "Simba", cat: "disney", img: "./img/017.png", pass: "D-RL30" },
        ...
        window.onload = function() {
            ...
            // ESCUCHA REALTIME
            const docRef = db.collection('artifacts').doc(appId).collection('public').doc('data').collection('store').doc('inventory');
        ...
        function renderizarCatalogo() {
            ...
            filtrados.forEach(t => {
                ...
                let btn = isAdmin ? `<div class="mt-4 pt-4 border-t border-zinc-800 space-y-2"><div class="flex justify-between items-center"><label class="text-[10px] font-bold">PRECIO</label><input type="number" value="${t.precio}" onchange="guardarPrecio(${t.id}, this)" class="w-16 bg-black border border-zinc-800 text-right text-xs p-1 text-white"></div><div class="bg-zinc-950 p-1 text-[10px] font-mono text-zinc-500">#${t.pass}</div><button onclick="toggleEstado(${t.id})" class="w-full py-2 text-[10px] font-black border uppercase tracking-widest ${esVendido ? 'border-green-900 text-green-500' : 'border-red-900 text-red-500'}">${esVendido ? 'Reactivar' : 'Marcar Vendido'}</button></div>` : 
                    (esVendido ? `<div class="mt-4 w-full py-3 bg-red-600 text-white font-black text-center text-xs tracking-widest shadow-[0_0_15px_rgba(220,38,38,0.4)]">VENDIDO</div>` : `<button onclick="abrirModalReserva(${t.id}, '${t.nombre}', ${t.precio}, null)" class="mt-4 w-full py-3 bg-white text-black font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 group hover:bg-red-600 hover:text-white transition-all">Reservar <i data-lucide="arrow-right" class="w-3 h-3 group-hover:translate-x-1 transition-transform"></i></button>`);
                
                grid.innerHTML += `<div class="bg-zinc-900 border border-zinc-800 hover:border-red-600/50 transition-all duration-300 relative ${esVendido && !isAdmin ? 'opacity-50 grayscale' : ''} shadow-lg"><div class="h-64 bg-black relative overflow-hidden flex items-center justify-center"><img src="${t.img}" class="w-full h-full object-cover opacity-90 group-hover:opacity-100" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex'"><div class="hidden flex items-center justify-center font-black text-red-900/10 text-6xl italic">BT</div><span class="absolute top-0 right-0 bg-black text-white text-[10px] px-3 py-1 font-mono border-l border-b border-zinc-800 z-10 opacity-40">#${t.id.toString().padStart(3,'0')}</span>${esVendido ? `<div class="absolute inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-20"><span class="text-3xl font-black text-red-600 uppercase border-4 border-red-600 px-6 py-2 -rotate-12 shadow-2xl">Vendido</span></div>` : ''}</div><div class="p-6"><h3 class="text-xl font-black text-white uppercase italic truncate mb-2">${t.nombre}</h3><div class="flex justify-between items-center border-b border-zinc-800 pb-4 mb-2"><span class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">${t.cat}</span>${!isAdmin ? `<span class="text-lg font-bold text-red-500 italic">${t.precio}€</span>` : ''}</div>${btn}</div></div>`;
            });
            lucide.createIcons();
        }
        ...
        function updateCloud(id, data) {
            const state = {}; tattoos.forEach(t => { state[t.id] = { estado: t.estado, precio: t.precio }; if(t.id === id) Object.assign(state[t.id], data); });
            db.collection('artifacts').doc(appId).collection('public').doc('data').collection('store').doc('inventory').set(state, {merge:true});
        }
        ...
        function procesarLogin() { if(document.getElementById('admin-pass').value === 'Vs2!3%95^2UhgKLavyRCSkWm$*vW6b') { isAdmin = true; cerrarModal('modal-login'); document.getElementById('admin-badge').classList.remove('hidden'); document.getElementById('btn-admin-lock').classList.add('text-red-500'); cambiarVista('home'); } else document.getElementById('login-error').classList.remove('hidden'); }
        ...
        function seleccionarCatRandom(c) {
            ...
            else { btn.innerText = "JUGAR AHORA"; btn.className = "w-full bg-white text-black font-black py-5 rounded uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all"; }
        }
        ...
        function cambiarVista(v) {
            ...
            document.getElementById('btn-home').className = v === 'home' ? 'px-8 py-2.5 rounded-full text-sm font-bold uppercase text-white bg-red-700 shadow-lg' : 'px-8 py-2.5 rounded-full text-sm font-bold uppercase text-zinc-400 hover:text-white hover:bg-zinc-800';
            document.getElementById('btn-random').className = v === 'random' ? 'px-8 py-2.5 rounded-full text-sm font-bold uppercase text-white bg-red-700 shadow-lg' : 'px-8 py-2.5 rounded-full text-sm font-bold uppercase text-zinc-400 hover:text-white hover:bg-zinc-800';
            if(v === 'random') volverMenuRandom(); else renderizarCatalogo();
        }
        function filtrarCategoria(c) { filtroActual = c; document.querySelectorAll('.filter-btn').forEach(b => b.className = b.dataset.cat === c ? 'filter-btn px-6 py-3 border border-zinc-800 text-xs font-black uppercase bg-white text-black' : 'filter-btn px-6 py-3 border border-zinc-800 text-xs font-black uppercase text-zinc-500 hover:text-white bg-zinc-900'); renderizarCatalogo(); }
